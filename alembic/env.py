...
from matosinhosgrocery.config import (
...
def list_tables_in_db(db_url_str: str):
    ...
    if not db_url_str.startswith("sqlite:///"):
        print(
            f"[DEBUG HOOK] DB URL '{db_url_str}' is not a recognized SQLite URL for direct connection. Skipping table list."
        )
        return
    ...
    if db_path.startswith("./"):
        ...
        project_root = (
            os.path.dirname(config.config_file_name)
    ...
    try:
        ...
        if tables:
            ...
            print(
                f"[DEBUG HOOK] No tables found in {db_path} (it might be empty or new)."
            )
    except sqlite3.Error as e:
        print(
            f"[DEBUG HOOK] SQLite error while trying to list tables in {db_path}: {e}"
        )
    finally:
        if conn:
            conn.close()
