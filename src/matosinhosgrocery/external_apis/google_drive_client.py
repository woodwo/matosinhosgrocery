...
def get_gdrive_service() -> Optional[Resource]:
    ...
    if not settings.GOOGLE_DRIVE_CREDENTIALS_PATH:
        logger.error("Google Drive credentials path is not configured. Cannot initialize GDrive service.")
        return None
    ...
    try:
        ...
        logger.error(f"Google Drive credentials file not found at: {settings.GOOGLE_DRIVE_CREDENTIALS_PATH}")
        return None
...
async def upload_file_to_gdrive(
    ...
    try:
        ...
        logger.info(f"Successfully uploaded file '{file_name_on_drive}' to Google Drive. File ID: {file_resource.get('id')}")
        return {"id": file_resource.get('id'), "webViewLink": file_resource.get('webViewLink')}
    except HttpError as error:
        logger.error(f"An HTTP error occurred while uploading to Google Drive: {error}")
        return None
    except Exception as e:
        logger.exception(f"An unexpected error occurred while uploading to Google Drive: {e}")
        return None
...
async def main_test():
    ...
    if not settings.GOOGLE_DRIVE_CREDENTIALS_PATH:
        print("Please set GOOGLE_DRIVE_CREDENTIALS_PATH in your .env or directly in settings for testing.")
        return
