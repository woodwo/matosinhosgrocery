...
from matosinhosgrocery.database.connection import (
    AsyncSessionLocal,
...
async def process_receipt_task(
    ...
    try:
        logger.info(
            "[DB_SESSION_DEBUG] process_receipt_task: Attempting to acquire DB session."
        )
        async with AsyncSessionLocal() as session:
            logger.info(
                f"[DB_SESSION_DEBUG] process_receipt_task: DB Session acquired: {session}, active: {session.is_active}"
            )
            service = ReceiptProcessingService(
                db_session=session, bot_instance=context.bot
            )
            ...
            logger.info(
                f"[DB_SESSION_DEBUG] process_receipt_task: Processing complete, attempting explicit commit. Session active: {session.is_active}"
            )
            await session.commit()
            logger.info(
                f"[DB_SESSION_DEBUG] process_receipt_task: Explicit commit successful. Session active: {session.is_active}"
            )

        # session should be committed and closed by context manager exiting here if no error before explicit commit
        logger.info(
            f"[DB_SESSION_DEBUG] process_receipt_task: Exited AsyncSessionLocal context. Receipt ID: {receipt.id if receipt else 'N/A'}"
        )
        ...
        logger.info(
            f"[DB_SESSION_DEBUG] process_receipt_task: Successfully sent confirmation message to user for receipt {receipt.id}"
        )

    except ValueError as ve:
        logger.error(
            f"[DB_SESSION_DEBUG] process_receipt_task: ValueError: {ve}. Session state before potential rollback: {session.is_active if session else 'N/A'}"
        )
        # Rollback should happen in __aexit__ of AsyncSessionLocal if an error propagates
        await context.bot.send_message(
            chat_id=chat_id, text=f"Error processing receipt: {ve}"
        )
    except Exception as e:
        logger.exception(
            f"[DB_SESSION_DEBUG] process_receipt_task: Unexpected Exception: {e}. Session state before potential rollback: {session.is_active if session else 'N/A'}"
        )
        ...
        await context.bot.send_message(
            chat_id=chat_id, text=error_message, parse_mode="HTML"
        )
    finally:
        ...
        if session:
            ...
            logger.info(
                f"[DB_SESSION_DEBUG] process_receipt_task: In finally block. Session active: {session.is_active}, dirty: {session.dirty}, new: {len(session.new)}"
            )
        else:
            logger.info(
                "[DB_SESSION_DEBUG] process_receipt_task: In finally block. Session was None (likely error before acquisition)."
            )
...
async def handle_receipt_document(
    ...
    if message.document:
        ...
        logger.info(
            f"User {user.id} sent document: {original_file_name} (ID: {file_id})."
        )
        await message.reply_text(
            f"Got your document: {original_file_name}. Processing it now..."
        )
        ...
        logger.warning(
            f"Receipt handler triggered for unexpected message type from user {user.id}"
        )
