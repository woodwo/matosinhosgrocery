...
class Receipt(Base, TimestampMixin):
    ...
    total_amount = sa.Column(
        sa.Float
    )  # Optional, might not always be available or accurate from OCR
    ...
    gdrive_filename = sa.Column(
        sa.String, nullable=True
    )  # The actual filename used on GDrive
    ...
    product_entries = relationship(
        "ProductEntry", back_populates="receipt", cascade="all, delete-orphan"
    )
