...
class ProductEntry(Base, TimestampMixin):
    ...
    original_name = sa.Column(sa.String, nullable=False, index=True) # As it appears on the receipt
    generalized_name = sa.Column(sa.String, nullable=False, index=True) # AI-processed name
    
    price_per_unit = sa.Column(sa.Float, nullable=False)
    quantity = sa.Column(sa.Float, default=1.0, nullable=False) # e.g., 1, 2, 0.5 (for kg)
    
    weight_volume_text = sa.Column(sa.String, nullable=True) # e.g., "120g", "1L", as text from receipt
    ...
    receipt_id = sa.Column(sa.Integer, sa.ForeignKey("receipts.id"), nullable=False, index=True)
