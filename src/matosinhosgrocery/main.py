...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "Attempting to create polling task (start_telegram_bot_polling will check if bot is initialized)..."
    )
    ...
    logger.info(
        f"Polling task created: {polling_task.get_name() if hasattr(polling_task, 'get_name') else 'N/A'}. Waiting for it to start logging..."
    )
...
app.include_router(
    receipt_routes.receipt_api_router, prefix="/api/v1", tags=["Receipts"]
)
